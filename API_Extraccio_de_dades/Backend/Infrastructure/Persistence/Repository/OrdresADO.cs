...
public class OrdresADO
{
    public static void Insert(DatabaseConnection dbConn, OrdreEntity ordre, List<LineaOrdreEntity> lineas)
    {
        ...
        try
        {
            ...
            string sqlOrdre = @"INSERT INTO ordenes (id, numero, fecha, fecha_recepcion, modo_pago, gestionado_por, direccion_entrega, total_ht, total_iva, total_ttc, moneda, id_proveedor, id_cliente, estado) 
                                VALUES (@id, @numero, @fecha, @fecha_recepcion, @modo_pago, @gestionado_por, @direccion_entrega, @total_ht, @total_iva, @total_ttc, @moneda, @id_proveedor, @id_cliente, @estado)";
            ...
            cmdOrdre.Parameters.AddWithValue("@estado", ordre.Estado);
            ...
            string sqlLinea = @"INSERT INTO lineas_orden (id, id_orden, descripcion, cantidad, precio_unitario, descuento, precio_neto, importe_ht, tva, codigo_cliente, codigo_proveedor)
                                VALUES (@id, @id_orden, @descripcion, @cantidad, @precio_unitario, @descuento, @precio_neto, @importe_ht, @tva, @codigo_cliente, @codigo_proveedor)";
            ...
            foreach (var linea in lineas)
            {
                ...
                cmdLinea.Parameters.AddWithValue("@codigo_cliente", linea.CodigoCliente);
                cmdLinea.Parameters.AddWithValue("@codigo_proveedor", linea.CodigoProveedor);
